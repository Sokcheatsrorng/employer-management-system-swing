...
import com.e6.entity.Payment;
import com.e6.entity.Payment.PaymentBuilder;
	...
	public List<Payment> list(Worker worker, String dateStrings){
		...
		if(tokenizer.countTokens() == 1) {
			...
			try {
				 ...
				 d1.setTime(d1.getTime() + 86400000L);
				 String date2 = new SimpleDateFormat(dbDatePattern).format(d1);
				 query = "SELECT * FROM worker_payment WHERE worker_id=" + worker.getId() + " AND date >= '" + date1 + "' AND date <= '" + date2 + "';";
			} catch (ParseException e) {
				e.printStackTrace();
			}
		} else {
